...
/*   Updated: 2025/06/10 17:30:17 by htharrau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	4 bytes		~±3.4E38				~7 digits	Sig. only	Decimals
double	8 bytes		~±1.7E308				~15 digits	Sig. only	Dec ++precis
...
int			std::stringstream			ss >> i;
			atoi						atoi(str.c_str())
			sscanf						sscanf(str.c_str(), "%d", &i)
float		std::stringstream			ss >> f;
			atof + cast					static_cast<float>(atof(str.c_str()))
			sscanf						sscanf(str.c_str(), "%f", &f)
double		std::stringstream			ss >> d;
			atof						atof(str.c_str())
			sscanf						sscanf(str.c_str(), "%lf", &d)
...
void ScalarConverter::handleDouble(const std::string& input, bool *handled) {
	...
	size_t posE = input.find('e');
	size_t posPt = input.find('.');
	if (posE == std::string::npos && posPt == std::string::npos)
		return ;
