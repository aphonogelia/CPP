...
Bureaucrat::~Bureaucrat() { 
	std::cout << "Bureaucrat destructor called on " << name << std::endl; 
}
...
void Bureaucrat::incrementGrade() { 
	if (grade == maxGrade) {
		std::string msg = "Cannot increment grade for " + name 
			+ ": already at maximum.";
...
void Bureaucrat::decrementGrade() {
	if (grade == minGrade) {
		std::string msg = "Cannot decrement grade for " + name 
			+ ": already at minimum";
...
void Bureaucrat::executeForm(const AForm& form) const {
	try {
		form.execute(*this); 
		std::cout << name << " executed " << form.getFormName() << " on " << form.getTarget() << std::endl;
	}
	catch (const std::exception& e) {
