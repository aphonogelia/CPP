...
const int AForm::defaultSEGrade = Bureaucrat::maxGrade;
 ...
 //		COPLIEN 	//
/////////////////////
AForm::AForm() : formName(defaultFName), target(defaultTarget), isSigned(false), 
	signGrade(defaultSEGrade), execGrade(defaultSEGrade) {
}

AForm::AForm(const std::string& inputName, const std::string& inputTarget, int inputSigGr, int inputExGr) 
		: formName(inputName), target(inputTarget), isSigned(false), signGrade(inputSigGr), execGrade(inputExGr) { 
...
AForm::AForm(const AForm& other) : formName(other.formName), 
	target(other.target), isSigned(other.isSigned),
...
AForm::~AForm() {
	std::cout << "AForm destructor called on " << formName << std::endl; 
}
...
std::ostream& operator<<(std::ostream& os, const AForm& f) {
	os << f.getFormName() << " (exec. grade " << f.getExecGrade() 
	<< ", sign. grade " << f.getSignGrade() << ", " 
	<< (f.getSignStatus() ? "": "not ") << "signed) has the following target: " << f.getTarget() 
	<< "." ;
	return os;
}
